...
ticketPriceTotal.push(runners.reduce((acc, curr) => { return acc += curr.donation }, 0));
...
// Problem 1
//We need a list of the biggest donors so we can contact them for more money .filter donations > 200
let generousPeops = [];
generousPeops.push(runners.filter(runner => runner.donation > 200));
console.log(generousPeops);
// Problem 2
//We need to extract the First Name and emails from this list so we can send the request for more money

//////////////////////////////////////////////////////////

let emails = [];
emails.push(generousPeops.forEach(peop => { return ({
...
// Problem 3
//CEO called in we need a total of runnner count ids in addition to the 1,000,000 runners he is importing from Mars we will be importing that many more runners from Venus for the solactic battleRun:
let runnerCount = [];
runnerCount.push(runners.reduce((acc, run) => {
    return acc += run.id;
}, 1000000));
